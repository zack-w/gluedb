...
describe Broker do
  describe '.find_or_create_broker' do
    it 'finds an existing broker by broker' do
      ...
      expect(Broker.find_or_create_broker(broker)).to eq exisiting_broker
    end

    it 'creates new broker if existing broker is not found' do
      new_broker = Broker.new(b_type: 'broker', npn: '666')
      expect(Broker.find_or_create_broker(new_broker)).to eq new_broker 
    end
  end
end
